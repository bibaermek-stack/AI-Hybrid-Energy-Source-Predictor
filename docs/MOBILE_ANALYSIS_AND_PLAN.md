...
### Зертханалар (12) — сайт және Android/iOS — ✅ жасалды

**Табылған ақаулар (сайт):**
- Барлық 11 зертханада «Рефлексия викторинасы» жіберілмейтін: `if st.button` ішінде тұрғандықтан, «Жіберу» басылғанда жоғалып кететін.
- Симуляция нәтижесі кез келген басқа батырманы басқанда жоғалатын.
- 11-зертхана (қуат ағыны) сайтта іске қосылмайтын — pandapower жоқ, тек мәтін.
- 12-зертхана (3D): модель бөліктерінің атаулары қате (DC кабельдер «есептегіш», инвертор «құрылым» деп белгіленген), текстуралар шатасқан, three.js unpkg.com-нан жүктелетін (жабық желіде 3D көрінбейтін), «Тексеру» бірден «Дұрыс» дейтін.
- Батарея 0 %-ға дейін разрядталатын (теориядағы DoD шегіне қайшы); бөлісілген энергия әдепкі параметрлерде әрқашан 0; батарея 0 кВт·сағ болса, зертхана құлайтын.
- Прогресс «12-нің 22-і» деп көрсететін (симуляция іске қосу зертхана аяқталды деп саналатын).

**Не жасалды:**
- Бір ортақ ядро (`src/education/labs/runner.py`): сайт пен қосымша бірдей параметр, есептеу және нәтиже алады.
- 11-зертхана: 0.4 кВ желідегі тәуліктік қуат ағыны (`src/simulation/grid/lv_feeder.py`, pandapower-сіз).
- 12-зертхана: нақты CAD моделі (DC ажыратқыш, DC кабельдер, инвертор, AC ажыратқыш, генерация есептегіші, логгер); «Зерттеу · Ақауды түзету · Тест» режимдері; 7 ақау сценарийі; инвертор экраны мен есептегіш күйді көрсетеді.
- Әр зертханаға тест: 5 сұрақ (3D-де 7), өту шегі 70 %. «Іске қосу» сұрақтарының жауабын зертхана ядросы есептейді; 3D сұрақтарына модельдегі бөлікті басып немесе жүйені түзетіп жауап береді.
- Мобильді қосымша: 12 зертхана тізімі, теория (формулалармен), параметр + іске қосу + графиктер, тест, 3D модель WebView ішінде (`flet-webview`). Android «Артқа» зертхана бетінен тізімге қайтарады.

**Ескерту:** қосымшаның зертханалары production API-ге сүйенеді — `main`-ге merge жасалып, Railway deploy болғаннан кейін ғана жұмыс істейді. Эмулятор тесті (`scripts/android_emulator_smoke.sh`) сол кезде 3D-ді де тексереді.
