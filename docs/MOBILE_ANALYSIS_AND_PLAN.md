...
### 1-кезең — Деректердің дұрыстығы (3–5 күн) — ✅ жасалды

**Күйі (2026-09-25):** барлық тармақ орындалды; 87/87 тест; веб-режимде әр экран жергілікті API-мен тексерілді.

| Экран | Бұрын | Қазір |
|---|---|---|
| Оңтайландыру | жоқ `optimal_dispatch` өрісін оқыды → қолжетімді қуатты «пайдаланылды» деп, батареяны 50 kW деп көрсетті | `/predict`-тің нақты өрістері: пайдаланылған/қолжетімді күн мен жел, батарея, желіден жетіспеушілік, curtailment, reliability |
| ML модель (Training) | R² 98.4%, RMSE 12.4, MAE 8.7 — ойдан | `GET /metrics`: R² 0.9967 / 0.9971 / 0.9137, YOLO mAP@50 93.1%, модельдердің нақты feature importance |
| Экология | 412.8 т, 18 650 ағаш, 245 т көмір, 88.4% — ойдан | `POST /sustainability/impact` (`src/sustainability`), ЖЭК мәнін инвертордан алу мүмкіндігі |
| Лаборатория | `s*0.85 + w*0.70`, «ПӘК» `92 + b/1000*6` | `POST /labs/microgrid-day` (`src/simulation`, дашбордтағы зертхана): PV, импорт/экспорт, өзіндік тұтыну, SoC, сағаттық график |
| ML Болжам | әдепкі 420.5 / 400.1 / 820.6 kW, ұсыныс ешқашан жаңармайды | «—» → модель жауабы; ұсыныс жаңарады; офлайн болса себебі |
| Басты / Solarman | демо деректер «Real-time» ретінде | «ДЕМО» белгісі; KPI «ML болжам (сценарий)» деп белгіленген |
| API клиент | офлайн кезде ойдан сан (`is_fallback`) | `None` + серверден келген нақты себеп (`detail`) |

Серверде Solarman кілттері қосылғанда «ДЕМО» белгісі өзі жоғалады.
...
### 2-кезең — UX, тіл, навигация (1–2 апта) — ✅ жасалды (камерадан басқасы)

**Күйі (2026-09-25):** 92/92 тест; веб-режимде KK/EN, қараңғы/жарық тақырып, телефон (393 px) және планшет (1000 px) ені тексерілді.

| Тармақ | Нәтиже |
|---|---|
| Навигация | 11 батырманың орнына 5 қойынды: **Басты · Болжам · Solarman · AI Кеңесші · Тағы**. «Болжам» — ML лездік + 24 сағат (SegmentedButton). «Тағы» — YOLO, Оңтайландыру, Экология, Зертхана, ML модельдер, Оқу, Баптаулар; ашылған экранда AppBar-да кері батырма. 600 px-тен кең экранда `NavigationRail` |
| Аударма | Барлық мәтін `mobile/i18n.py`-да (kk, en) жұп ретінде — 300-ден астам кілт. Тест: кілттердің бәрі бар, `{placeholder}`-лар екі тілде бірдей, экрандар кодында бірде-бір қазақша жол қалмаған |
| Тіл/тақырып ауысқанда | Экрандар кэштен өшіріліп, жаңа тілде/түсте қайта құрылады (бұрын тек AppBar өзгеретін). Чат тарихы `state`-те сақталады |
| Баптауларды сақтау | Тіл мен тақырып `SharedPreferences`-та; қайта ашқанда қалпына келеді (тексерілді) |
| Header күйі | Health 2 с-тан кеш келсе де AppBar жаңарады; «Онлайн/Офлайн» белгісін басса — қайта тексереді |
| Слайдерлер | Solarman PR/ROI сұрауы тек `on_change_end` кезінде (бұрын әр қадамда) |
| Ақау суреті | Телефонда `compression_quality=80`; 10 MB-тан үлкен болса серверге жібермей тұрып ескертеді |
| AI кеңесші | Офлайн кезде «жергілікті режим жұмыс істейді» деген жалған жауап жоқ — нақты себеп көрсетіледі |

**Кейін қосылды (2026-09-26):**
- **Камера:** ресми `flet-camera==1.0.1` кеңейтуі. Ақау экранында «Камера» батырмасы (тек Android/iOS; веб/desktop алдын ала қарауда кеңейту жоқ, сондықтан жасырылған): алдын ала көрініс → «Түсіру» → YOLO. Артқы камера, 720p, дыбыссыз (микрофон рұқсаты сұралмайды). Android-та `CAMERA` рұқсаты (камерасы жоқ құрылғыларға да орнатылады), iOS-та `NSCameraUsageDescription`. Рұқсат берілмесе — себебі мен галереяға кеңес.
- **Android «Артқа» батырмасы:** `View.can_pop` + `on_confirm_pop` — «Тағы» ішіндегі экраннан → «Тағы»; басқа қойындыдан → Басты; Бастыда → қосымшадан шығу. Ереже `nav_bar.back_target()`-та, тестпен бекітілген.
- Екеуін де нақты телефонда тексеру керек (эмулятор/браузер бұл батырма мен камераны толық көрсетпейді).

**Ескерту:**
- Веб-режимде `state` бір Python процесінде ортақ — бұл тек браузерлік алдын ала қарау үшін маңызды, телефонда әр қосымшаның өз процесі.
